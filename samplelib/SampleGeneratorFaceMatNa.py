...
class SampleGeneratorFaceMatNa(SampleGeneratorBase):
    def __init__ (self, paths, debug=False, batch_size=1, resolution=256, face_type=None,
        ...
        if len(seg_sample_idxs) == 0:
            seg_sample_idxs = SegmentedSampleFilterSubprocessor(samples, count_matna_mask=True).run()
            if len(seg_sample_idxs) == 0:
                raise Exception(f"No segmented faces found.")
            else:
                io.log_info(f"Using {len(seg_sample_idxs)} matna labeled samples.")
        else:
    ...
    def batch_func(self, param ):
        ...
        def gen_img_mask(sample):
            ...
            elif sample.has_matna_mask():
                mask = sample.get_matna_mask()
                mask[mask < 0.5] = 0.0
                mask[mask >= 0.5] = 1.0
            else:
...
class SegmentedSampleFilterSubprocessor(Subprocessor):
    #override
    def __init__(self, samples, count_matna_mask=False ):
        self.samples = samples
        self.samples_len = len(self.samples)
        self.count_matna_mask = count_matna_mask
    ...
    def process_info_generator(self):
        for i in range(multiprocessing.cpu_count()):
            yield 'CPU%d' % (i), {}, {'samples':self.samples, 'count_matna_mask':self.count_matna_mask}
    ...
    class Cli(Subprocessor.Cli):
        ...
        def on_initialize(self, client_dict):
            ...
            self.count_matna_mask = client_dict['count_matna_mask']

        def process_data(self, idx):
            if self.count_matna_mask:
                return idx, self.samples[idx].has_matna_mask()
            else:
