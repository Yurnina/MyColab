    ...
    def load_face_samples ( image_paths):
        ...
        for filename, data in result:
            ...
            sample_list.append( Sample(filename=filename,
                                        ...
                                        matna_mask_compressed=matna_mask_compressed,
...
class FaceSamplesLoaderSubprocessor(Subprocessor):
    ...
    def on_result (self, host_dict, data, result):
        idx, lfdimg = result
        self.result[idx] = (self.image_paths[idx], lfdimg)
    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            idx, filename = data
            lfdimg = LFDIMG.load (Path(filename))

            if lfdimg is None or not lfdimg.has_data():
                self.log_err (f"FaceSamplesLoader: {filename} is not a image file.")
                data = None
            else:
                data = (lfdimg.get_face_type(),
                        lfdimg.get_shape(),
                        lfdimg.get_landmarks(),
                        lfdimg.get_seg_ie_polys(),
                        lfdimg.get_matna_mask_compressed(),
                        lfdimg.get_eyebrows_expand_mod(),
                        lfdimg.get_source_filename() )
