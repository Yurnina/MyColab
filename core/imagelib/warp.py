...
def mls_rigid_deformation(vy, vx, src_pts, dst_pts, alpha=1.0, eps=1e-8):
	...
	reshaped_p = src_pts.reshape(ctrls, 1, 2, 1)
	reshaped_v = np.vstack((vx.reshape(1, grow, gcol), vy.reshape(1, grow, gcol)))
	w = 1.0 / (np.sum((reshaped_p - reshaped_v).astype(np.float32) ** 2, axis=2) + eps) ** alpha
	w /= np.sum(w, axis=0, keepdims=True)
	pstar = np.zeros((grow, gcol, 2), np.float32)
	for i in range(ctrls):
		pstar += w[i:i+1,...] * reshaped_p[i,...]
	vpstar = reshaped_v - pstar
	reshaped_mul_right = np.concatenate((vpstar[...,None],
		np.concatenate((vpstar[...,1:2], -vpstar[...,0:1]), axis=-1)
	), axis=-2).transpose(1, 2, 3, 0)
	reshaped_q = dst_pts.reshape((ctrls, 1, 2, 1))

	qstar = np.zeros((grow, gcol, 2), np.float32)
	for i in range(ctrls):
		qstar += w[i:i+1,...] * reshaped_q[i,...]
	temp = np.zeros((grow, gcol, 2), np.float32)
	for i in range(ctrls):
		phat = reshaped_p[i,...] - pstar
		qhat = reshaped_q[i,...] - qstar
		temp += np.matmul(qhat.transpose(2, 3, 0, 1),
			np.matmul((w[i:i+1,...,None] *
			np.concatenate((phat[...,None], np.concatenate((phat[...,1:2], -phat[...,0:1]), axis=-1)), axis=-2)
			), reshaped_mul_right)
		).transpose(2, 3, 0, 1)
	...
	transformers = np.zeros((grow, gcol, 2), np.float32)
	transformers[~nan_mask] = np.true_divide(temp[~nan_mask], normed_temp[~nan_mask]) * normed_vpstar[~nan_mask] + qstar[~nan_mask]
	...
	transformers[...,1][nan_mask] = np.interp(nan_mask_flat, nan_mask_anti_flat, transformers[...,0][~nan_mask])
	return transformers
...
def gen_warp_params(w, flip=False, rotation_range=(-10, 10), scale_range=(-0.5, 0.5), tx_range=(-0.05, 0.05), ty_range=(-0.05, 0.05), rnd_state=None, warp_rnd_state=None):
    ...
    if warp_rnd_state is None:
        warp_rnd_state = np.random.RandomState()
    rw = None
    if w < 64:
        rw = w
        w = 64
    rotation = rnd_state.uniform(*rotation_range)
    scale = rnd_state.uniform(1 + scale_range[0], 1 + scale_range[1])
    tx = rnd_state.uniform(*tx_range)
    ty = rnd_state.uniform(*ty_range)
    p_flip = flip and rnd_state.randint(10) < 4
    cell_size = w // (2 ** warp_rnd_state.randint(3))
    cell_count = w // cell_size + 1
    grid_points = np.linspace(0, w, cell_count)
    mapx = np.broadcast_to(grid_points, (cell_count, cell_count)).copy()
    mapy = mapx.T
    mapx[1:-1, 1:-1] = mapx[1:-1, 1:-1] + warp_rnd_state.normal(size=(cell_count - 2, cell_count - 2)) * (cell_size * 0.24)
    mapy[1:-1, 1:-1] = mapy[1:-1, 1:-1] + warp_rnd_state.normal(size=(cell_count - 2, cell_count - 2)) * (cell_size * 0.24)
...
def warp_by_params(params, img, can_warp, can_transform, can_flip, border_replicate, cv2_inter=cv2.INTER_CUBIC):
    ...
    if can_warp:
        img = cv2.remap(img, params['mapx'], params['mapy'], cv2_inter, borderMode=cv2.BORDER_CONSTANT)
    if can_transform:
    ...
    if img.ndim == 2:
        img = img[..., None]
    if can_flip and params.get('flip', False):
        img = img[:, ::-1, ...]
    return img
