    ...
    class Cli(Subprocessor.Cli):
        ...
        def process_data(self, data):
            ...
            if lfdimg is None or not lfdimg.has_data():
                self.log_err (f"{filepath.name} is not a image file")
                return [ str(filepath), 0 ]
            else:
                image = cv2_imread( str(filepath) )
                
                face_mask = LandmarksProcessor.get_image_hull_mask (image.shape, lfdimg.get_landmarks())
...
def sort_by_face_yaw(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading"):
        ...
        if lfdimg is None or not lfdimg.has_data():
            io.log_err (f"{filepath.name} is not a image file")
            trash_img_list.append ( [str(filepath)] )
            continue

        pitch, yaw, roll = LandmarksProcessor.estimate_pitch_yaw_roll ( lfdimg.get_landmarks(), size=lfdimg.get_shape()[1] )
...
def sort_by_face_source_rect_size(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading"):
        ...
        source_rect = lfdimg.get_source_rect()
...
def sort_by_origname(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading"):
        ...
        img_list.append( [str(filepath), lfdimg.get_source_filename()] )
        ...
        def process_data(self, data):
            ...
            try:
                ...
                if self.faster:
                    ...
                    face_mask = LandmarksProcessor.get_image_hull_mask (gray.shape, lfdimg.get_landmarks())     
