...
from facelib import FaceEnhancer, FaceType, LandmarksProcessor, MatNaNet
...
def main (model_class_name=None,
    ...
    try:
        ...
        matna_256_extract_func = MPClassFuncOnDemand(MatNaNet, 'extract',
                                                    name='MatNa',
                                                    resolution=256,
            ...
            if packed_samples is not None:
                ...
                def generator():
                    for sample in io.progress_bar_generator( packed_samples, "Collecting alignments"):
                        ...
                        yield filepath, LFDIMG.load(filepath, loader_func=lambda x: sample.read_raw_file()  )
            else:
                def generator():
                    for filepath in io.progress_bar_generator( pathex.get_image_paths(aligned_path), "Collecting alignments"):
                        filepath = Path(filepath)
                        yield filepath, LFDIMG.load(filepath)

            alignments = {}
            multiple_faces_detected = False

            for filepath, lfdimg in generator():
                if lfdimg is None or not lfdimg.has_data():
                    io.log_err (f"{filepath.name} is not a lfd image file")
                    continue

                source_filename = lfdimg.get_source_filename()
                ...
                alignments_ar.append ( (lfdimg.get_source_landmarks(), filepath, source_filepath ) )
        ...
        if len(frames) == 0:
            ...
            if False:
                ...
                InteractiveMergerSubprocessor (
                            ...
                            matna_256_extract_func = matna_256_extract_func,
                            merger_config          = cfg,
                            frames                 = frames,
...
for filepath in io.progress_bar_generator(input_path_image_paths, "Collecting info"):
    ...
    lfdimg = LFDIMG.x(filepath)
    if lfdimg is None:
        io.log_err ("%s is not a lfd image file" % (filepath.name) )
        continue
    filesdata += [ ( FrameInfo(filepath=filepath, landmarks_list=[lfdimg.get_landmarks()] ), lfdimg.get_source_filename() ) ]
