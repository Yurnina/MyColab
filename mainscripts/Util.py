...
def save_faceset_metadata_folder(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Processing"):
        ...
        if lfdimg is None or not lfdimg.has_data():
            io.log_info(f"{filepath} is not a image file")
            continue
            
        lfd_dict = lfdimg.get_dict()
        d[filepath.name] = ( lfdimg.get_shape(), lfd_dict )
...
def recover_original_aligned_filename(input_path):
    ...
    for filepath in io.progress_bar_generator( pathex.get_image_paths(input_path), "Processing"):
        ...
        files += [ [filepath, None, lfdimg.get_source_filename(), False] ]
