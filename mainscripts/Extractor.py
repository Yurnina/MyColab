    ...
    class Cli(Subprocessor.Cli):
        ...
        def final_stage(data,
            ...
            for rect, image_landmarks in zip( rects, landmarks ):
                ...
                lfdimg = LFDJPG.load(output_filepath)
                lfdimg.set_face_type(FaceType.toString(face_type))
                lfdimg.set_landmarks(face_image_landmarks.tolist())
                lfdimg.set_source_filename(filepath.name)
                lfdimg.set_source_rect(rect)
                lfdimg.set_source_landmarks(image_landmarks.tolist())
...
def main(detector=None,
    ...
    if face_type is None:
        if manual_output_debug_fix:
            ...
            if len(files) != 0:
                lfdimg = LFDIMG.load(Path(files[0]))
    ...
    if face_type is None:
        face_type = io.input_str ("Face type", 'wf', ['f','wf','head'], help_message="Full face / whole face / head. 'Whole face' covers full area of face include forehead. 'head' covers full head, but requires MatNa for src and dst faceset.").lower()
