...
def MergeMaskedFace (predictor_func, predictor_input_shape,
    ...
    elif cfg.mask_mode >= 6 and cfg.mask_mode <= 9:  #MatNa modes
        if cfg.mask_mode == 6 or cfg.mask_mode == 8 or cfg.mask_mode == 9:
            # obtain MatNa-prd
            prd_face_matna_bgr = cv2.resize (prd_face_bgr, (matna_input_size,)*2, interpolation=cv2.INTER_CUBIC)
            prd_face_matna_mask = matna_256_extract_func(prd_face_matna_bgr)
            X_prd_face_mask_a_0 = cv2.resize ( prd_face_matna_mask, (output_size, output_size), interpolation=cv2.INTER_CUBIC)

        if cfg.mask_mode >= 7 and cfg.mask_mode <= 9:
            # obtain MatNa-dst
            matna_mat            = LandmarksProcessor.get_transform_mat (img_face_landmarks, matna_input_size, face_type=cfg.face_type)
            dst_face_matna_bgr   = cv2.warpAffine(img_bgr, matna_mat, (matna_input_size,)*2, flags=cv2.INTER_CUBIC )
            dst_face_matna_mask  = matna_256_extract_func(dst_face_matna_bgr)
            X_dst_face_mask_a_0 = cv2.resize (dst_face_matna_mask, (output_size,output_size), interpolation=cv2.INTER_CUBIC)

        if cfg.mask_mode == 6:   #'MatNa-prd'
            wrk_face_mask_a_0 = X_prd_face_mask_a_0
        elif cfg.mask_mode == 7: #'MatNa-dst'
            wrk_face_mask_a_0 = X_dst_face_mask_a_0
        elif cfg.mask_mode == 8: #'MatNa-prd*MatNa-dst'
            wrk_face_mask_a_0 = X_prd_face_mask_a_0 * X_dst_face_mask_a_0
        elif cfg.mask_mode == 9: #learned-prd*learned-dst*MatNa-prd*MatNa-dst
...
def MergeMasked (predictor_func,
    ...
    for face_num, img_landmarks in enumerate( frame_info.landmarks_list ):
        out_img, out_img_merging_mask = MergeMaskedFace (predictor_func, predictor_input_shape, face_enhancer_func, matna_256_extract_func, cfg, frame_info, img_bgr_uint8, img_bgr, img_landmarks)
