...
class QCanvasControlsRightBar(QFrame):
    ...
    def __init__(self):
        ...
        btn_view_matna_mask = QToolButton()
        self.btn_view_matna_mask_act = QActionEx( QIconDB.view_matna, QStringDB.btn_view_matna_mask_tip, shortcut='5', shortcut_in_tooltip=True, is_checkable=True)
        btn_view_matna_mask.setDefaultAction(self.btn_view_matna_mask_act)
        btn_view_matna_mask.setIconSize(QUIConfig.icon_q_size)

        btn_view_matna_overlay_mask = QToolButton()
        self.btn_view_matna_overlay_mask_act = QActionEx( QIconDB.view_matna_overlay, QStringDB.btn_view_matna_overlay_mask_tip, shortcut='`', shortcut_in_tooltip=True, is_checkable=True)
        btn_view_matna_overlay_mask.setDefaultAction(self.btn_view_matna_overlay_mask_act)
        btn_view_matna_overlay_mask.setIconSize(QUIConfig.icon_q_size)
        ...
        self.btn_poly_color_act_grp.addAction(self.btn_view_matna_mask_act)
        ...
        controls_bar_frame2_l = QVBoxLayout()
        controls_bar_frame2_l.addWidget ( btn_view_matna_overlay_mask )
        controls_bar_frame2 = QFrame()
        controls_bar_frame2.setFrameShape(QFrame.StyledPanel)
        controls_bar_frame2.setSizePolicy (QSizePolicy.Fixed, QSizePolicy.Fixed)
        controls_bar_frame2.setLayout(controls_bar_frame2_l)
        ...
        controls_bar_frame1_l.addWidget ( btn_view_matna_mask )
...
class QCanvasOperator(QWidget):
    def __init__(self, cbar):
        ...
        self.cbar.btn_view_matna_mask_act.triggered.connect ( lambda : self.set_op_mode(OpMode.VIEW_MATNA_MASK) )

        self.cbar.btn_view_matna_overlay_mask_act.toggled.connect ( lambda is_checked: self.update() )
    ...
    def initialize(self, img, img_look_pt=None, view_scale=None, ie_polys=None, matna_mask=None, canvas_config=None ):
        q_img = self.q_img = QImage_from_np(img)
        self.img_pixmap = QPixmap.fromImage(q_img)

        self.matna_mask_pixmap = None
        self.matna_overlay_mask_pixmap = None
        if matna_mask is not None:
            h,w,c = img.shape
            matna_mask = cv2.resize(matna_mask, (w,h), interpolation=cv2.INTER_CUBIC)
            matna_mask = imagelib.normalize_channels(matna_mask, 1)
            matna_img = img.astype(np.float32)/255.0
            matna_overlay_mask = matna_img*(1-matna_mask)*0.5 + matna_img*matna_mask
            matna_overlay_mask = np.clip(matna_overlay_mask*255, 0, 255).astype(np.uint8)
            matna_mask = np.clip(matna_mask*255, 0, 255).astype(np.uint8)
            self.matna_mask_pixmap = QPixmap.fromImage(QImage_from_np(matna_mask))
            self.matna_overlay_mask_pixmap = QPixmap.fromImage(QImage_from_np(matna_overlay_mask))
        ...
        self.cbar.btn_view_matna_overlay_mask_act.setDisabled(False)
    ...
    def finalize(self):
        if self.initialized:
            ...
            self.last_state = sn(op_mode = self.op_mode if self.op_mode in [OpMode.VIEW_BAKED, OpMode.VIEW_MATNA_MASK] else None,
    ...
    def set_op_mode(self, op_mode, op_poly=None):
        ...
        if self.op_mode != op_mode:
            ...
            elif self.op_mode == OpMode.VIEW_MATNA_MASK:
                self.cbar.btn_view_matna_mask_act.setChecked(False)
            ...
            elif op_mode == OpMode.VIEW_MATNA_MASK:
                self.cbar.btn_view_matna_mask_act.setChecked(True)
    ...
    def set_cbar_disabled(self):
        ...
        self.cbar.btn_view_matna_overlay_mask_act.setDisabled(True)
        self.cbar.btn_poly_type_act_grp.setDisabled(True)


    def set_color_scheme_id(self, id):
        if self.op_mode == OpMode.VIEW_BAKED or self.op_mode == OpMode.VIEW_MATNA_MASK:
            self.set_op_mode(OpMode.NONE)
    ...
    def paintEvent(self, event):
        ...
        if self.op_mode == OpMode.VIEW_BAKED:
            ...
            if self.matna_mask_pixmap is not None:
                qp.drawPixmap(dst_rect, self.matna_mask_pixmap, src_rect)
        else:
            if self.cbar.btn_view_matna_overlay_mask_act.isChecked() and \
               self.matna_overlay_mask_pixmap is not None:
                qp.drawPixmap(dst_rect, self.matna_overlay_mask_pixmap, src_rect)
...
class QCanvas(QFrame):
    def __init__(self):
        ...
        cbar = sn( btn_poly_color_red_act   = self.canvas_control_right_bar.btn_poly_color_red_act,
                   ...
                   btn_view_matna_mask_act  = self.canvas_control_right_bar.btn_view_matna_mask_act,
                   btn_view_matna_overlay_mask_act  = self.canvas_control_right_bar.btn_view_matna_overlay_mask_act,
...
class LoaderQSubprocessor(QSubprocessor):
    ...
    def on_result (self, host_dict, data, result):
        idx, has_lfdimg, has_ie_polys = result
    ...
    class Cli(QSubprocessor.Cli):
        def process_data(self, data):
            idx, filename = data
            lfdimg = LFDIMG.load(filename)
            if lfdimg is not None and lfdimg.has_data():
...
class MainWindow(QXMainWindow):
    ...
    def canvas_initialize(self, image_path, only_has_polys=False):
        ...
        lfdimg = LFDIMG.load(image_path)
        if not lfdimg or not lfdimg.has_data():
            return False
        ...
        self.canvas.op.initialize ( img,  ie_polys=ie_polys, matna_mask=matna_mask )
    ...
    def canvas_finalize(self, image_path):
        ...
        if image_path.exists():
            ...
            if not new_ie_polys.identical(ie_polys):
                ...
                lfdimg.set_seg_ie_polys( new_ie_polys )
                lfdimg.save()
...
def start(input_dirpath):
    ...
    returns exit_code
    """
    io.log_info("Running MatNa editor.")
    ...
    app = QApplication([])
    app.setApplicationName("MatNaEditor")
    app.setStyle('Fusion')
