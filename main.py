if __name__ == "__main__":
    ...
    def process_exportmfd(arguments):
        osex.set_process_lowest_prio()
        from mainscripts import ExportMFD
        ExportMFD.main(model_class_name = arguments.model_name, saved_models_path = Path(arguments.model_dir))

    p = subparsers.add_parser( "exportmfd", help="Export model to use in mmLive.")
    p.add_argument('--model-dir', required=True, action=fixPathAction, dest="model_dir", help="Saved models dir.")
    p.add_argument('--model', required=True, dest="model_name", choices=pathex.get_all_dir_names_startswith ( Path(__file__).parent / 'models' , 'Model_'), help="Model class name.")
    p.set_defaults (func=process_exportmfd)
    ...
    p = facesettool_parser.add_parser ("enhance", help="Enhance details in faceset.")
    ...
    p = facesettool_parser.add_parser ("resize", help="Resize faceset.")
    p.add_argument('--input-dir', required=True, action=fixPathAction, dest="input_dir", help="Input directory of aligned faces.")
    ...
    # ========== MatNa
    matna_parser = subparsers.add_parser( "matna", help="MatNa tools.").add_subparsers()
    
    p = matna_parser.add_parser( "editor", help="MatNa editor.")

    def process_matnaeditor(arguments):
        osex.set_process_lowest_prio()
        from MatNaEditor import MatNaEditor
        global exit_code
        exit_code = MatNaEditor.start (Path(arguments.input_dir))
        
    p.add_argument('--input-dir', required=True, action=fixPathAction, dest="input_dir")

    p.set_defaults (func=process_matnaeditor)
  
    p = matna_parser.add_parser( "apply", help="Apply trained MatNa model to the extracted faces.")

    def process_matnaapply(arguments):
        osex.set_process_lowest_prio()
        from mainscripts import MatNaUtil
        MatNaUtil.apply_matna (Path(arguments.input_dir), Path(arguments.model_dir))
    p.add_argument('--input-dir', required=True, action=fixPathAction, dest="input_dir")
    p.add_argument('--model-dir', required=True, action=fixPathAction, dest="model_dir")
    p.set_defaults (func=process_matnaapply)
    
    
    p = matna_parser.add_parser( "remove", help="Remove applied MatNa masks from the extracted faces.")
    def process_matnaremove(arguments):
        osex.set_process_lowest_prio()
        from mainscripts import MatNaUtil
        MatNaUtil.remove_matna (Path(arguments.input_dir) )
    p.add_argument('--input-dir', required=True, action=fixPathAction, dest="input_dir")
    p.set_defaults (func=process_matnaremove)
    
    
    p = matna_parser.add_parser( "remove_labels", help="Remove MatNa labels from the extracted faces.")
    def process_matnaremovelabels(arguments):
        osex.set_process_lowest_prio()
        from mainscripts import MatNaUtil
        MatNaUtil.remove_matna_labels (Path(arguments.input_dir) )
    p.add_argument('--input-dir', required=True, action=fixPathAction, dest="input_dir")
    p.set_defaults (func=process_matnaremovelabels)
    
    
    p = matna_parser.add_parser( "fetch", help="Copies faces containing MatNa polygons in <input_dir>_matna dir.")

    def process_matnafetch(arguments):
        osex.set_process_lowest_prio()
        from mainscripts import MatNaUtil
        MatNaUtil.fetch_matna (Path(arguments.input_dir) )
    p.add_argument('--input-dir', required=True, action=fixPathAction, dest="input_dir")
    p.set_defaults (func=process_matnafetch)
