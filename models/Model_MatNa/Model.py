...
from facelib import FaceType, MatNaNet
from models import ModelBase
from samplelib import *

class MatNaModel(ModelBase):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, force_model_class_name='MatNa', **kwargs)
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run():
            self.options['face_type'] = io.input_str ("Face type", default_face_type, ['h','mf','f','wf','head'], help_message="Half / mid face / full face / whole face / head. Choose the same as your model.").lower()
    ...
    def on_initialize(self):
        ...
        self.model = MatNaNet(name='MatNa',
        ...
        if self.is_training:
            ...
            if self.pretrain:
                ...
                srcdst_generator = SampleGeneratorFaceMatNa([self.training_data_src_path, self.training_data_dst_path],
                                                            debug=self.is_debug(),
    ...
    def onGetPreview(self, samples, for_history=False):
        ...
        result += [ ('MatNa training faces', np.concatenate (st, axis=0 )), ]
        ...
        if not self.pretrain and len(src_samples) != 0:
            ...
            result += [ ('MatNa src faces', np.concatenate (st, axis=0 )), ]
        ...
        if not self.pretrain and len(dst_samples) != 0:
            ...
            result += [ ('MatNa dst faces', np.concatenate (st, axis=0 )), ]
